...
# 构造保存文件文件名
path = f'{date}_{time}'.replace('-', '_').replace(':', '_')
# 发起请求
a = requests.get('https://www.weibo.com/ajax/side/hotSearch', headers=header)
# 解析为json
response = json.loads(a.text)
data = response['data']['realtime']
# 写入csv
with open(f"./data/{path}.csv", 'a', newline='', encoding='utf-8') as f:
    writer = csv.DictWriter(f, fieldnames=['date', 'time', 'hot_index', 'name', 'raw_hot', 'label_name', 'url'])
    # 写入表头（只在文件为空时写入）
    if f.tell() == 0:
        writer.writeheader()
    for i in data:
...
# 将JSON数据写入文件
with open(f"./data/{path}.txt", "w", encoding="utf-8") as file:
